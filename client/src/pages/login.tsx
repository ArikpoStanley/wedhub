...
export default function Login() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await refresh();
      const next = getNextPath(typeof window !== "undefined" ? window.location.search : "");
      navigate(next);
    } catch (e) {
      setError(
        e instanceof Error
          ? e.message
          : "Could not reach the server. Confirm `npm run dev` is running and use the same URL the server prints (e.g. http://localhost:5000).",
      );
    } finally {
      setBusy(false);
    }
